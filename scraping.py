...
def main():
    ...
    for index in range(10):
        x = threading.Thread(target=scrape_world, args=(index,))
        threads.append(x)
        x.start()
...
def scrape_world(args):
    ...
    driver.get(f"https://randomstreetview.com/")
    ...
    index = 0
    num_scraped = 0
    while num_scraped < 1000:
        location_data = driver.execute_script(f"return randomLocations.all")
        ...
        country = location_data[index]['formatted_address'].split(",")[-1].strip()
